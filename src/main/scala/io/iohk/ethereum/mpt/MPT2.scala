...
import io.iohk.ethereum.mpt.MPT2.HashFn
...
object MPT2 {
  ...
  def apply[K, V](rootNode: Node, hashFn: HashFn)
                 (implicit kSerializer: ByteArraySerializable[K], vSerializer: ByteArraySerializable[V])
  : MPT2[K, V] = new MPT2[K, V](rootNode, hashFn)(kSerializer, vSerializer)

  private def matchingLength(a: Array[Byte], b: Array[Byte]): Int = a.zip(b).takeWhile(t => t._1 == t._2).length

  private def getNextNode(extensionNode: ExtensionNode)(implicit nodeDec: RLPDecoder[Node]): Node =
    extensionNode.next match {
      case Right(node) => node
      case Left(hash) => decode[Node](hash)
    }

  private def getChild(branchNode: BranchNode, pos: Int)(implicit nodeDec: RLPDecoder[Node]): Option[Node] =
  ...
  case class MPTUpdate(previousRoot: Node,
                       newRoot: Option[Node],
                       toRemove: Seq[Node],
                       toUpdate: Seq[Node],
                       hashFn: HashFn)
...
class MPT2[K, V](val rootNode: Node, private val hashFn: HashFn)
                (implicit kSerializer: ByteArraySerializable[K], vSerializer: ByteArraySerializable[V]) {
  ...
  private[mpt] implicit val nodeDec = new RLPDecoder[Node] {
    override def decode(rlp: RLPEncodeable): Node = rlp match {
      case RLPList(items@_*) if items.size == MPT2.ListSize =>
      ...
      case RLPList(items@_*) if items.size == MPT2.PairSize =>
  ...
  /**
    ...
    * @throws MPTException if there is any inconsistency in how the trie is build.
    */
  def put(key: K, value: V): (MPTUpdate, MPT2[K, V]) = {
    val keyNibbles = HexPrefix.bytesToNibbles(kSerializer.toBytes(key))
    val NodeInsertResult(newRoot, nodesToRemoveFromStorage, nodesToUpdateInStorage) = put(rootNode, keyNibbles, vSerializer.toBytes(value))
    val newMPT = new MPT2(newRoot, hashFn)(kSerializer, vSerializer)
    val mPTUpdate = MPTUpdate(rootNode, Some(newRoot), nodesToRemoveFromStorage, nodesToUpdateInStorage, hashFn)
    mPTUpdate -> newMPT
  }
  ...
  /**
    ...
    * @throws MPTException if there is any inconsistency in how the trie is build.
    */
  // TODO ugly return type. we need to be able to easily define empty update and empty MPT
  def remove(key: K): (Option[MPTUpdate], Option[MPT2[K, V]]) = {
    val keyNibbles = HexPrefix.bytesToNibbles(bytes = kSerializer.toBytes(key))
    remove(rootNode, keyNibbles) match {
      case NodeRemoveResult(true, Some(newRootNode), nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
        val mPTUpdate = MPTUpdate(rootNode, Some(newRootNode), nodesToRemoveFromStorage, nodesToUpdateInStorage, hashFn)
        val newMPT = new MPT2(newRootNode, hashFn)(kSerializer, vSerializer)
        Some(mPTUpdate) -> Some(newMPT)
      case NodeRemoveResult(true, None, nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
        val mPTUpdate = MPTUpdate(rootNode, None, nodesToRemoveFromStorage, nodesToUpdateInStorage, hashFn)
        Some(mPTUpdate) -> None // TODO should be mptUpdate -> MPT2.Empty
      case NodeRemoveResult(false, _, _, _) =>
        None -> Some(this) // TODO should be MPTUpdate.Empty -> this
    }
  }
  ...
  private def get(node: Node, searchKey: Array[Byte]): Option[Array[Byte]] = node match {
    ...
    case branch@BranchNode(_, terminator, _) =>
      ...
      else getChild(branch, searchKey(0)) match {
    ...
    matchingLength(sharedKey, searchKey) match {
      ...
      case ml if ml == sharedKey.length =>
        ...
        val NodeInsertResult(newChild: BranchNode, toDeleteFromStorage, toUpdateInStorage) =
          put(getNextNode(extensionNode), searchKey.drop(ml), value)
    ...
    if (searchKey.isEmpty) {
      ...
      if (children(searchKeyHead).isDefined) {
        ...
        val NodeInsertResult(changedChild, toDeleteFromStorage, toUpdateInStorage) =
          put(getChild(branchNode, searchKeyHead).get, searchKeyRemaining, value)
  ...
  private def removeFromBranchNode(node: BranchNode, searchKey: Array[Byte]): NodeRemoveResult = (node, searchKey.isEmpty) match {
    ...
    case (BranchNode(children, _, _), true) =>
      ...
      val fixedNode = fix(BranchNode(children, None, hashFn), Seq())
      ...
      getChild(branchNode, searchKeyHead) map { child =>
        ...
        remove(child, searchKey.tail) match {
          case NodeRemoveResult(true, maybeNewChild, nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
            ...
            val fixedNode = fix(nodeToFix, nodesToUpdateInStorage)
            NodeRemoveResult(
              hasChanged = true,
  ...
  private def removeFromExtensionNode(extensionNode: ExtensionNode, searchKey: Array[Byte]): NodeRemoveResult = {
    ...
    if (cp == sharedKey.length) {
      ...
      remove(getNextNode(extensionNode), searchKey.drop(cp)) match {
        case NodeRemoveResult(true, maybeNewChild, nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
          ...
          maybeNewChild map { newChild =>
            ...
            val fixedNode = fix(toFix, nodesToUpdateInStorage)
            NodeRemoveResult(
              hasChanged = true,
              newNode = Some(fixedNode),
  ...
  /**
    ...
    */
  @tailrec
  private def fix(node: Node, notStoredYet: Seq[Node]): Node = node match {
